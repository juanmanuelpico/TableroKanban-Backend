...
public interface ProyectoRolUsuarioRepository extends JpaRepository<ProyectoRolUsuario, Integer> {
    ...
    @Query(value = "SELECT p.id_proyecto, p.nombre, p.descripcion, p.activo, p.fecha_inicio, p.fecha_fin FROM proyecto_rol_usuario pru INNER JOIN proyecto p ON pru.id_proyecto = p.id_proyecto WHERE " +
            "pru.id_usuario = :id_usuario", nativeQuery = true)
